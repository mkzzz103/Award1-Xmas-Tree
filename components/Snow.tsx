...
// Bypassing JSX.IntrinsicElements errors by using local constants for 3D primitives
const T = {
...
const Snow: React.FC<{ mixFactor: number }> = ({ mixFactor }) => {
  ...
  return (
    <T.Points ref={pointsRef} frustumCulled={false}>
      <T.BufferGeometry>
        <T.BufferAttribute attach="attributes-position" count={count} array={positions} itemSize={3} />
        <T.BufferAttribute attach="attributes-aScale" count={count} array={scales} itemSize={1} />
        <T.BufferAttribute attach="attributes-aVelocity" count={count} array={velocities} itemSize={3} />
      </T.BufferGeometry>
      <T.ShaderMaterial 
        ref={materialRef}
